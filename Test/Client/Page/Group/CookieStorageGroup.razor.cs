...
public sealed partial class CookieStorageGroup : ComponentBase {
    ...
    public const string OUTPUT = "cookie-storage-output";
    ...
    private async Task GetAllCookies_Property() {
        output = await CookieStorage.AllCookies;
    }
    ...
    private async Task GetAllCookies_Method() {
        output = await CookieStorage.GetAllCookies(CancellationToken.None);
    }
    ...
    private async Task Key() {
        output = await CookieStorage.Key(TEST_KEY_INDEX) ?? $"key {TEST_KEY_INDEX} is not present";
    }

    public const string BUTTON_GET_COOKIE = "cookie-storage-get-cookie";
    private async Task GetCookie() {
        output = await CookieStorage.GetCookie(TEST_GET_COOKIE) ?? $"key '{TEST_GET_COOKIE}' is not present";
    }
