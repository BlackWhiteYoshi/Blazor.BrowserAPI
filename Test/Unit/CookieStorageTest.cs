...
public sealed class CookieStorageTest : PlayWrightTest {
    public CookieStorageTest(PlayWrightFixture playWrightFixture) : base(playWrightFixture) { }
    ...
    public async Task GetAllCookies_Property(int number) {
        ...
        await Page.GetByTestId(CookieStorageGroup.DATA_TESTID_GET_ALL_COOKIES_PROPERTY).ClickAsync();
    ...
    public async Task GetAllCookies_Method(int number) {
        ...
        await Page.GetByTestId(CookieStorageGroup.DATA_TESTID_GET_ALL_COOKIES_METHOD).ClickAsync();
    ...
    public async Task GetLength_Property(int number) {
        ...
        await Page.GetByTestId(CookieStorageGroup.DATA_TESTID_GET_LENGTH_PROPERTY).ClickAsync();
    ...
    public async Task GetLength_Method(int number) {
        ...
        await Page.GetByTestId(CookieStorageGroup.DATA_TESTID_GET_LENGTH_METHOD).ClickAsync();
    ...
    public async Task Key() {
        ...
        await Page.GetByTestId(CookieStorageGroup.DATA_TESTID_KEY).ClickAsync();
    ...
    public async Task GetCookie() {
        ...
        await Page.GetByTestId(CookieStorageGroup.DATA_TESTID_GET_COOKIE).ClickAsync();
    ...
    public async Task SetCookie() {
        await Page.GetByTestId(CookieStorageGroup.DATA_TESTID_SET_COOKIE).ClickAsync();
    ...
    public async Task RemoveCookie() {
        ...
        await Page.GetByTestId(CookieStorageGroup.DATA_TESTID_REMOVE_COOKIE).ClickAsync();
    ...
    public async Task Clear(int number) {
        ...
        await Page.GetByTestId(CookieStorageGroup.DATA_TESTID_CLEAR).ClickAsync();
    ...
    [InlineData(3)]
    [InlineData(10)]
    public async Task GetAllCookies_InProcess(int number) {
        string expected = string.Empty;
        if (number > 0) {
            StringBuilder builder = new();
            for (int i = 0; i < number; i++)
    ...
    [InlineData(3)]
    [InlineData(10)]
    public async Task GetLength_InProcess(int number) {
        for (int i = 0; i < number; i++)
            await Page.EvaluateAsync($"document.cookie = 'test-key-{i}=test-value-{i}'");
    ...
    [Fact]
    public async Task Key_InProcess() {
        const string KEY = "test-key-0";
        await Page.EvaluateAsync($"document.cookie = '{KEY}=test-value-0'");
    ...
    [Fact]
    public async Task GetCookie_InProcess() {
        const string VALUE = "test-getCOOKIE-value";
        await Page.EvaluateAsync($"document.cookie = '{CookieStorageGroup.GET_COOKIE_TEST}={VALUE}'");
    ...
    [Fact]
    public async Task SetCookie_InProcess() {
    ...
    [Fact]
    public async Task RemoveCookie_InProcess() {
        await Page.EvaluateAsync($"document.cookie = '{CookieStorageGroup.REMOVE_COOKIE_TEST}=test-value'");
    ...
    [InlineData(3)]
    [InlineData(10)]
    public async Task Clear_InProcess(int number) {
        for (int i = 0; i < number; i++)
            await Page.EvaluateAsync($"document.cookie = 'test-key-{i}=test-value-{i}'");
