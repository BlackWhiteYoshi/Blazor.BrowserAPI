...
public sealed class LocalStorageInProcess(IModuleManager moduleManager) : ILocalStorageInProcess {
    ...
    /// <returns></returns>
    public string? Key(int index) => moduleManager.InvokeSync<string?>("LocalStorageAPI.key", [index]);
    ...
    /// <returns></returns>
    public string? GetItem(string key) => moduleManager.InvokeSync<string?>("LocalStorageAPI.getItem", [key]);
    ...
    public void SetItem(string key, string value) => moduleManager.InvokeSync("LocalStorageAPI.setItem", [key, value]);
    ...
    public void RemoveItem(string key) => moduleManager.InvokeSync("LocalStorageAPI.removeItem", [key]);

    /// <summary>
    /// When invoked, will empty all keys out of localStorage.
    /// </summary>
    public void Clear() => moduleManager.InvokeSync("LocalStorageAPI.clear");
}
