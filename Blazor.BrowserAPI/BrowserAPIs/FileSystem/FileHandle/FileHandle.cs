...
public sealed class FileHandle(IJSObjectReference fileHandleJS) : FileHandleBase(fileHandleJS), IFileHandle {
    [AutoInterfaceVisibilityInternal]
    IJSObjectReference IFileHandle.FileHandleJS => fileHandleJS;

    /// <summary>
    /// Releases the JS instance for this file handle.
    /// </summary>
    /// <returns></returns>
    public ValueTask DisposeAsync() => fileHandleJS.DisposeTrySync();
    ...
    /// <param name="cancellationToken"></param>
    /// <returns></returns>
    public ValueTask<bool> IsSameEntry(IFileHandle other, CancellationToken cancellationToken = default) => IsSameEntryBase(other.FileHandleJS, cancellationToken);
    ...
    /// <param name="cancellationToken"></param>
    /// <returns></returns>
    public async ValueTask<IFile> GetFile(CancellationToken cancellationToken = default) {
        IJSObjectReference fileJS = await GetFileBase(cancellationToken);
        return new File(fileJS);
    }
    ...
    /// <param name="cancellationToken"></param>
    /// <returns></returns>
    public async ValueTask<IWritableFileStream> CreateWritable(bool keepExistingData = false, string mode = "siloed", CancellationToken cancellationToken = default) {
        IJSObjectReference fileJS = await CreateWritableBase(keepExistingData, mode, cancellationToken);
        return new WritableFileStream(fileJS);
    }
}
