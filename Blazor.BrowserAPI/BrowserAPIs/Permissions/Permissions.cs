...
public sealed class Permissions(IModuleManager moduleManager) : IPermissions {
    /// <summary>
    ...
    /// <returns></returns>
    public async ValueTask<IPermissionStatus> Query(string name, bool userVisibleOnly = false, bool sysex = false, CancellationToken cancellationToken = default) {
        IJSObjectReference permissionStatusJS = await moduleManager.InvokeAsync<IJSObjectReference>("PermissionsAPI.query", cancellationToken, [name, userVisibleOnly, sysex]);
