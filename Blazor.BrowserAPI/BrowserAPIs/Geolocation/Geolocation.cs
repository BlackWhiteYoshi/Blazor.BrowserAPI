...
public sealed class Geolocation(IModuleManager moduleManager) : IGeolocation {
    /// <summary>
    ...
    /// <returns></returns>
    public ValueTask GetCurrentPosition(Action<GeolocationCoordinates, long> successCallback, Action<int, string>? errorCallback = null, long maximumAge = 0, long timeout = -1, bool enableHighAccuracy = false, CancellationToken cancellationToken = default) {
        DotNetObjectReference<CallbackGeolocation> callbackGeolocation = DotNetObjectReference.Create(new CallbackGeolocation(null!));
        callbackGeolocation.Value.SuccessCallback = (GeolocationCoordinates geolocationCoordinates, long timestamp) => {
            successCallback(geolocationCoordinates, timestamp);
